...
import java.awt.event.*;
	...
	public class JTableTest{	
		...
		DefaultTableModel mod=new DefaultTableModel(data,field);	// 用field和data来定义表格模板
		JTable tab=new JTable(mod);			// 用模板mod来创建表格
		...
		void go(){
			...
			col.setFont(ft);val.setFont(ft);tab.setFont(ft);
	    		tab.addMouseListener(new TableH());	// 将表格注册到监听器TableH中，它实现了接口MouseListener
		}
		...
		class ButtonH implements ActionListener{
			...
			public void actionPerformed(ActionEvent e){
				if(sel==1){								// 当按了"插入"按钮时
					...
					mod.addRow(dt);					// 在模板mod的末尾添加一行
				}
				if(sel==2)								// 当按了"删除"按钮时
					mod.removeRow(tab.getSelectedRow());//在模板mod中删除表格tab中的当前行
			}
		}
